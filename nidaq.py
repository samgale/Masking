...
class DigitalOutput(BaseTask):
    ...
    def cfg_drive_type(self,
                       drive_type,
                       ):
        """ Configure drive type to 'Active drive' or 'open collector'.
        """
        if drive_type in ['o', 'open']:
            const = DAQmx_Val_OpenCollector
        elif drive_type in ['a', 'active']:
            const = DAQmx_Val_ActiveDrive
        self.SetDOOutputDriveType(self.devstr, const)
